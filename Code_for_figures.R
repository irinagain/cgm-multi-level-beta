...
p = ranges %>% gather(key = "type", value = "value", -idnew, -id)%>% ggplot(aes(x = idnew, y = value, group = idnew)) + geom_line() + geom_point() + xlab("Subject") + ylab("Glucose [mg/dL]")
print(p)
dev.off()
...
p = ggplot(selectedMeans,aes(x = t, y = smean)) + geom_line(aes(group=idnew), col = "blue")  + geom_line(aes(x = t, y = rawmean, group=idnew), data = selectedRawMeans, col = "red")+ xlab("Hours of sleep") + ylab("Glucose  [mg / dL]") + ylim(c(50, 300))
print(p)
dev.off()  
...
p = ggplot(smoothed_means,aes(x = sleeptime, y = gl)) + geom_line(aes(group=id), col = "grey")  + geom_line(aes(group=id), data = smoothed_means[smoothed_means$id %in% selectedID,], col = "blue")+ xlab("Hours of sleep") + ylab("Glucose  [mg / dL]") + ylim(c(50, 300))
print(p)
dev.off()  
...
p = ggplot(datagl,aes(x = as.numeric(sleeptime), y = gl)) + geom_line(aes(group=sleepday)) + facet_wrap(~as.character(idnew), ncol = 2)+ xlim(c(0,7))+ xlab("Hours of sleep") + ylab("Glucose  [mg / dL]") + ylim(c(50, 400))+ theme(legend.position="none")
print(p)
dev.off()
