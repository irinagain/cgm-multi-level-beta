...
########################################################
library(readxl)
newa1c.path = '../Punjabi_Hypnos_2018_01_23.xlsx'
...
p1 = dexcom%>%ggplot(aes(x = scores1, y = a1c)) + geom_point(alpha = 0.3, size = 2) + xlab("Mean score 1") + ylab(expression(paste("HbA"["1c"]," %"))) + geom_point(aes(scores1, a1c, col = id), data = dexcom_selected, size = 3)+ theme(legend.position="none")
p2 = dexcom%>%ggplot(aes(x = scores_sd1, y = a1c)) + geom_point(alpha = 0.3, size = 2) + xlab("Sd score 1") + ylab(expression(paste("HbA"["1c"]," %"))) + geom_point(aes(scores_sd1, a1c, col = id), data = dexcom_selected, size = 3)+ theme(legend.position="none")
...
# Calculate correlations with all other scores and a1c -> all below 0.21
cor(dexcom$a1c, dexcom$scores2, use = "complete.obs") # -0.19
cor(dexcom$a1c, dexcom$scores3, use = "complete.obs") # 0.10
cor(dexcom$a1c, dexcom$scores_sd2, use = "complete.obs") #-0.21
